...
async function seed() {
  ...
  await db.insert(announcementsTable).values([
    ...
    { id: "ann_plat_01", courseId: null, authorId: "user_superadmin", title: "New: built-in learner supports", body: "Lessons now include read-aloud narration, on-demand translation, and adjustable, guided pacing, so every learner can work in the way that suits them best.", platformWide: true, publishedAt: ago(2), createdAt: ago(2) },
  ]).onConflictDoNothing();
