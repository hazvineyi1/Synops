...
<style>:root{--indigo:#4F46E5;--amber:#F59E0B;--ink:#1f2430;--ok:#15803d;--no:#b91c1c}*{box-sizing:border-box}body{font-family:Inter,system-ui,sans-serif;color:var(--ink);margin:0;padding:18px;background:#FBF7EF;overflow-x:hidden}h2{margin:.2rem 0 1rem;font-size:1.25rem}.q{background:#fff;border:2px solid #f0e9da;border-radius:20px;padding:16px 18px;margin:0 0 16px;box-shadow:0 2px 8px rgba(0,0,0,.05)}.q.solved{border-color:var(--ok);background:#f6fbf7}.qt{font-weight:700;font-size:1.1rem;margin:0 0 12px}.qimg{display:block;width:170px;height:170px;object-fit:contain;border-radius:22px;margin:0 auto 12px;background:#eef0fb;padding:10px;filter:drop-shadow(0 5px 9px rgba(0,0,0,.18))}.opt{display:flex;align-items:center;gap:10px;width:100%;text-align:left;border:2px solid #e6e0d0;background:#fff;border-radius:14px;padding:14px 16px;margin:8px 0;font:inherit;font-size:1.1rem;font-weight:700;cursor:pointer;transition:.15s}.opt:hover{border-color:var(--indigo);transform:translateY(-1px)}.opt .mk{margin-left:auto;font-size:1.3rem}.opt.correct{border-color:var(--ok);background:#e9f7ee;animation:pop .5s}.opt.wrong{border-color:var(--no);background:#fdecec;animation:shake .4s}@keyframes pop{0%{transform:scale(1)}40%{transform:scale(1.08)}70%{transform:scale(.97)}100%{transform:scale(1)}}@keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-7px)}75%{transform:translateX(7px)}}.bar{height:12px;background:#eee;border-radius:8px;overflow:hidden;margin:16px 0 8px}.fill{height:100%;width:0;background:linear-gradient(90deg,#F59E0B,#15803d);transition:.4s}.hint{color:#6b7280;font-size:1rem;text-align:center;font-weight:600}.done{display:none;text-align:center;font-weight:800;font-size:1.5rem;color:var(--ok);margin:14px 0}.arrow{display:inline-block;animation:bob 1s infinite}@keyframes bob{0%,100%{transform:translateY(0)}50%{transform:translateY(7px)}}.float{position:fixed;pointer-events:none;font-size:1.9rem;z-index:9;animation:rise 1.4s ease-out forwards}@keyframes rise{0%{transform:translateY(0) scale(.5);opacity:0}20%{opacity:1}100%{transform:translateY(-150px) scale(1.25) rotate(18deg);opacity:0}}.confetti{position:fixed;top:-30px;font-size:1.8rem;animation:fall linear forwards;z-index:9;pointer-events:none}@keyframes fall{to{transform:translateY(110vh) rotate(360deg);opacity:.85}}</style>
<h2>${title}</h2><div id="app"></div><div class="bar"><div class="fill" id="f"></div></div><p id="s" class="hint">Tap the right answer! 👆</p><div id="done" class="done">🎉 You did it! <span class="arrow">👇</span> Tap Continue!</div>
<script>const items=${data};const app=document.getElementById('app');const solved=new Array(items.length).fill(false);let doneCount=0;function report(s){try{parent.postMessage({type:'activity_result',score:s,payload:{solved}},'*');}catch(e){}}function burst(el,kind){const r=el.getBoundingClientRect();const set=kind==='balloon'?['🎈','⭐','🎉','✨']:['🎉','⭐','🌟','🎊','✨','🏆'];const n=kind==='balloon'?9:32;for(let i=0;i<n;i++){const s=document.createElement('span');if(kind==='balloon'){s.className='float';s.style.left=(r.left+Math.random()*r.width)+'px';s.style.top=r.top+'px';s.style.animationDelay=(Math.random()*.25)+'s';}else{s.className='confetti';s.style.left=(Math.random()*100)+'vw';s.style.animationDuration=(1.6+Math.random()*1.4)+'s';s.style.animationDelay=(Math.random()*.4)+'s';}s.textContent=set[i%set.length];document.body.appendChild(s);setTimeout(()=>s.remove(),3600);}}function bar(){document.getElementById('f').style.width=Math.round(doneCount/items.length*100)+'%';}function finish(){document.getElementById('s').style.display='none';document.getElementById('done').style.display='block';burst(document.body,'confetti');setTimeout(()=>report(100),1300);}items.forEach((it,qi)=>{const d=document.createElement('div');d.className='q';d.innerHTML=(it.img?'<img class="qimg" src="'+it.img+'" alt="picture">':'')+'<p class="qt">'+(qi+1)+'. '+it.q+'</p>';it.options.forEach((o,oi)=>{const b=document.createElement('button');b.className='opt';b.innerHTML='<span>'+o+'</span><span class="mk"></span>';b.onclick=()=>{if(solved[qi])return;const mk=b.querySelector('.mk');if(oi===it.answer){b.classList.add('correct');mk.textContent='✅';solved[qi]=true;doneCount++;[...d.querySelectorAll('.opt')].forEach(x=>{if(x!==b)x.disabled=true;});d.classList.add('solved');burst(b,'balloon');bar();if(doneCount===items.length)setTimeout(finish,450);}else{b.classList.add('wrong');mk.textContent='❌';setTimeout(()=>{b.classList.remove('wrong');mk.textContent='';},700);}};d.appendChild(b);});app.appendChild(d);});</script>`;
}
