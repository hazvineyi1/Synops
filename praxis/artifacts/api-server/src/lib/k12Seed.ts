...
function gameHtml(title: string, items: { q: string; options: string[]; answer: number; img?: string }[], mode: string): string {
...
async function createK12Course(c: K12Course, orgId: string, facultyId: string): Promise<string> {
  ...
  for (let mi = 0; mi < c.modules.length; mi++) {
    ...
    await db.insert(interactiveActivitiesTable).values({
      ...
      html: gameHtml(`${m.title}: quick check`, m.quiz, m.game ?? "choice"), source: "html", kind: "quiz",
  ...
  for (const c of COURSES) {
    ...
    for (let i = 0; i < cmods.length && i < c.modules.length; i++) {
      ...
      await db.update(interactiveActivitiesTable)
        .set({ html: gameHtml(`${m.title}: quick check`, m.quiz, m.game ?? "choice") })
