...
<style>:root{--indigo:#4F46E5;--amber:#F59E0B;--ink:#1f2430;--ok:#15803d;--no:#b91c1c}*{box-sizing:border-box}body{font-family:Inter,system-ui,sans-serif;color:var(--ink);margin:0;padding:18px;background:#FBF7EF}h2{margin:.2rem 0 1rem;font-size:1.15rem}.q{background:#fff;border:2px solid #f0e9da;border-radius:20px;padding:16px 18px;margin:0 0 16px;box-shadow:0 2px 8px rgba(0,0,0,.05)}.qt{font-weight:700;font-size:1.1rem;margin:0 0 12px}.opt{display:flex;align-items:center;gap:10px;width:100%;text-align:left;border:2px solid #e6e0d0;background:#fff;border-radius:14px;padding:14px 16px;margin:8px 0;font:inherit;font-size:1.05rem;font-weight:600;cursor:pointer;transition:.15s}.opt:hover{border-color:var(--indigo);transform:translateY(-1px)}.opt .mk{margin-left:auto;font-size:1.3rem}.opt.sel{border-color:var(--indigo);background:#eef0fb}.opt.correct{border-color:var(--ok);background:#e9f7ee;animation:pop .5s}.opt.wrong{border-color:var(--no);background:#fdecec;animation:shake .4s}@keyframes pop{0%{transform:scale(1)}40%{transform:scale(1.06)}70%{transform:scale(.97)}100%{transform:scale(1)}}@keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-6px)}75%{transform:translateX(6px)}}.bar{height:10px;background:#eee;border-radius:6px;overflow:hidden;margin:14px 0 6px}.fill{height:100%;width:0;background:var(--amber);transition:.4s}button.go{background:var(--indigo);color:#fff;border:0;border-radius:10px;padding:11px 18px;font:inherit;font-weight:600;cursor:pointer}.score{font-weight:700;font-size:1.05rem;margin:4px 0}.hint{color:#6b7280;font-size:.9rem}.qimg{display:block;width:170px;height:170px;object-fit:contain;border-radius:22px;margin:0 auto 12px;background:#eef0fb;padding:10px;filter:drop-shadow(0 5px 9px rgba(0,0,0,.18))}.confetti{position:fixed;top:-30px;font-size:1.7rem;animation:fall linear forwards;z-index:9;pointer-events:none}@keyframes fall{to{transform:translateY(108vh) rotate(360deg);opacity:.85}}</style>
...
<script>const items=${data};const app=document.getElementById('app');const picks=new Array(items.length).fill(-1);let done=false;items.forEach((it,qi)=>{const d=document.createElement('div');d.className='q';d.innerHTML=(it.img?'<img class="qimg" src="'+it.img+'" alt="picture to read">':'')+'<p class="qt">'+(qi+1)+'. '+it.q+'</p>';it.options.forEach((o,oi)=>{const b=document.createElement('button');b.className='opt';b.innerHTML='<span>'+o+'</span><span class="mk"></span>';b.onclick=()=>{if(done)return;picks[qi]=oi;[...d.querySelectorAll('.opt')].forEach(x=>x.classList.remove('sel'));b.classList.add('sel');upd();};d.appendChild(b);});app.appendChild(d);});function upd(){const n=picks.filter(p=>p>=0).length;document.getElementById('f').style.width=Math.round(n/items.length*100)+'%';}function report(s){try{parent.postMessage({type:'activity_result',score:s,payload:{picks}},'*');}catch(e){}}function party(){const e=['🎉','⭐','🌟','🎊','✨','🏆'];for(let i=0;i<30;i++){const s=document.createElement('span');s.className='confetti';s.textContent=e[i%e.length];s.style.left=(Math.random()*100)+'vw';s.style.animationDuration=(1.6+Math.random()*1.4)+'s';s.style.animationDelay=(Math.random()*.4)+'s';document.body.appendChild(s);setTimeout(()=>s.remove(),3600);}}document.getElementById('submit').onclick=()=>{if(done)return;let right=0;const qs=[...document.querySelectorAll('.q')];items.forEach((it,qi)=>{const opts=[...qs[qi].querySelectorAll('.opt')];opts.forEach((b,oi)=>{b.disabled=true;const mk=b.querySelector('.mk');if(oi===it.answer){b.classList.add('correct');if(mk)mk.textContent='✅';}if(picks[qi]===oi&&oi!==it.answer){b.classList.add('wrong');if(mk)mk.textContent='❌';}});if(picks[qi]===it.answer)right++;});const pct=Math.round(right/items.length*100);done=true;document.getElementById('f').style.width='100%';const good=pct>=75;document.getElementById('s').innerHTML='<span class="score">'+(good?'🎉 ':'')+'You got '+right+' of '+items.length+' right ('+pct+'%). '+(good?'Amazing work!':'Good try — look again!')+'</span>';if(good)party();report(pct);};</script>`;
}
