...
export interface Mail {
  ...
  html: string;
  text?: string;
  /** Override the From display name (address stays the verified EMAIL_FROM sender). */
  fromName?: string;
}

/** Tenant branding applied to an email (resolved from brand_themes; see lib/emailBrand). */
...
/** Build the From header, swapping in a tenant display name while keeping the verified address. */
function fromHeader(fromName?: string): string {
  ...
  return clean && addr ? `${clean} <${addr}>` : base;
}
...
export function emailShell(opts: {
  ...
  const accent = brand.primaryColor || DEFAULT_BRAND.primaryColor;
  const name = escapeAttr(brand.displayName || DEFAULT_BRAND.displayName);
  const header = brand.logoUrl
    ? `<img src="${escapeAttr(brand.logoUrl)}" alt="${name}" height="28" style="height:28px;max-width:200px;object-fit:contain;display:block" />`
    : `<div style="font-weight:800;font-size:16px;color:${accent}">${name}</div>`;
  const cta =
    opts.ctaLabel && opts.ctaUrl
      ? `<tr><td style="padding:8px 0 4px"><a href="${opts.ctaUrl}" style="display:inline-block;background:${accent};color:#fff;text-decoration:none;padding:10px 18px;border-radius:8px;font-weight:600;font-size:14px">${opts.ctaLabel}</a></td></tr>`
      : "";
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="padding:24px 0"><tr><td align="center">
    <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #dce4e1;border-radius:14px;overflow:hidden">
      ...
      <tr><td style="padding:20px 24px 0">${header}</td></tr>
    ...
    <div style="font-size:11px;color:#8a9995;padding:12px 0">You're receiving this because of your role in ${name}.</div>
  </td></tr></table>
  </body></html>`;
}
