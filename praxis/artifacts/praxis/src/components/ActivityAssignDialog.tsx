...
import { activitiesApi, type AssignTier, type ActivityAssignmentRow } from "@/lib/activitiesApi";
...
/** Assign an activity one tier down the Partner -> Organisation -> Learner chain. */
export function ActivityAssignDialog({ activityId, activityTitle, onClose }: { activityId: string; activityTitle: string; onClose: () => void }) {
  ...
  const targetsQ = useQuery({ queryKey: ["act-assign-targets", activityId], queryFn: () => activitiesApi.assignTargets(activityId) });
  const assignmentsQ = useQuery({ queryKey: ["act-assignments", activityId], queryFn: () => activitiesApi.assignments(activityId) });
  ...
  const assign = useMutation({
    mutationFn: () => activitiesApi.assign(activityId, { tier, targetIds: [...selected], groupId: group ?? undefined, dueDate: dueDate ? new Date(dueDate).toISOString() : null }),
    onSuccess: (r) => {
      qc.invalidateQueries({ queryKey: ["act-assignments", activityId] });
      qc.invalidateQueries({ queryKey: ["act-assign-targets", activityId] });
  ...
  const revoke = useMutation({
    mutationFn: (id: string) => activitiesApi.revokeAssignment(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["act-assignments", activityId] }); qc.invalidateQueries({ queryKey: ["act-assign-targets", activityId] }); },
    onError: (e: Error) => toast({ title: "Could not revoke", description: e.message, variant: "destructive" }),
  });

  const active = useMemo(() => (assignmentsQ.data ?? []).filter((a) => a.status !== "revoked"), [assignmentsQ.data]);
      ...
      <div className="relative w-full max-w-lg max-h-[85vh] overflow-auto rounded-xl bg-white shadow-xl border">
        <div className="sticky top-0 bg-white border-b px-5 py-3.5 flex items-center justify-between">
          <div className="min-w-0">
            <p className="text-xs uppercase tracking-wider text-muted-foreground">Assign activity</p>
            <h2 className="font-semibold truncate">{activityTitle}</h2>
          </div>
        ...
        <div className="p-5 space-y-5">
          {targetsQ.isLoading ? (
            ...
            <p className="text-sm text-muted-foreground">Your role cannot assign this activity.</p>
          ) : (
            <>
              <div>
          ...
          <div className="pt-1">
            ...
            {assignmentsQ.isLoading ? <Skeleton className="h-8 rounded-md" /> : active.length === 0 ? (
              <p className="text-sm text-muted-foreground">Not assigned to anyone yet.</p>
            ) : (
              <div className="space-y-1.5">
                {active.map((a: ActivityAssignmentRow) => (
