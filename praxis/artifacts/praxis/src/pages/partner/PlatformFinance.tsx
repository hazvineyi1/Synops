...
import { platformOverview, allPartners, ZAR } from '@/lib/partnerHubData';
import {
  useFilings, addFiling, removeFiling, setFilingStatus, FILING_TYPES,
  type FilingType, type FilingStatus,
} from '@/lib/platformFilingStore';
...
export function PlatformFinance() {
  ...
  const partnerNames = useMemo(() => allPartners().map((p) => p.name), []);
  ...
  const [upPartner, setUpPartner] = useState<string>(partnerNames[0] ?? 'Platform');
  ...
  const submitUpload = () => {
    ...
    addFiling({ title: upTitle.trim(), docType: upType, partner: upPartner, counterparty: upCounter.trim() || '-', status: 'active', signed: new Date().toISOString().slice(0, 10), expires: '', size: upFileName ? 'uploaded' : '-' });
    setUpOpen(false); setUpTitle(''); setUpCounter(''); setUpFileName('');
    flashMsg('Filed to the cabinet.');
  };
  ...
  const expiringCount = filings.filter((f) => f.status === 'expiring' || f.status === 'expired').length;

  return (
    <div className="space-y-6">
      <PageHeader title="Financial Hub" icon={Wallet}
        subtitle="Platform-wide financials across every partner, and the organised cabinet for contracts and MOUs." />
      ...
      <Tabs defaultValue="financials">
        ...
        <TabsContent value="financials" className="mt-4 space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <StatCard icon={Receipt} label="Outstanding" value={ZAR(totals.outstanding)} tint={totals.overdue ? 'bg-amber-500/10 text-amber-600' : 'bg-muted text-muted-foreground'} />
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
                <tr>
                  ...
                  <th className="text-right p-3">Funder received</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {partners.map((p) => (
                  <tr key={p.id} className="hover:bg-muted/20">
                    ...
                    <td className="p-3 text-right tabular-nums">{ZAR(p.funderReceived)}</td>
                  </tr>
                ))}
                <tr className="bg-muted/30 font-semibold">
                  ...
                  <td className="p-3 text-right tabular-nums">{ZAR(totals.funderReceived)}</td>
                </tr>
              </tbody>
            </table>
          </Card>
        ...
        {/* ── Contracts & MOUs (filing cabinet) ──────────── */}
        <TabsContent value="filing" className="mt-4 space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <StatCard icon={FileText} label="Documents on file" value={filings.length} tint="bg-indigo-500/10 text-indigo-600" />
            <StatCard icon={FolderOpen} label="MOUs" value={filings.filter((f) => f.docType === 'MOU').length} tint="bg-violet-500/10 text-violet-600" />
            <StatCard icon={Building} label="Contracts / MSAs" value={filings.filter((f) => f.docType === 'MSA' || f.docType === 'SLA' || f.docType === 'Partnership').length} tint="bg-emerald-500/10 text-emerald-600" />
          ...
          <Card className="p-4 space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search title, counterparty, partner"
            ...
            {upOpen && (
              <div className="rounded-lg border border-border p-3 grid sm:grid-cols-2 gap-2 bg-muted/20">
                ...
                <div className="sm:col-span-2 flex items-center gap-2">
                  ...
                  <Button size="sm" className="gap-1.5 ml-auto" disabled={!upTitle.trim()} onClick={submitUpload}><CheckCircle2 className="h-3.5 w-3.5" /> File it</Button>
                </div>
              </div>
            )}
          </Card>
                ...
                {filtered.map((f) => {
                  ...
                  return (
                    <tr key={f.id} className="hover:bg-muted/20">
                      ...
                      <td className="p-3">
                        <select value={f.status} onChange={(e) => setFilingStatus(f.id, e.target.value as FilingStatus)}
                      ...
                      <td className="p-3 text-right"><Button size="sm" variant="ghost" className="h-8 text-muted-foreground hover:text-red-600" onClick={() => removeFiling(f.id)}><Trash2 className="h-3.5 w-3.5" /></Button></td>
                    </tr>
                  );
                })}
                {filtered.length === 0 && (
                  <tr><td colSpan={7} className="p-6 text-center text-sm text-muted-foreground">No documents match your filters.</td></tr>
                )}
              </tbody>
            </table>
          </Card>
