...
import {
  getPartnerHub, orgDetail, orgCourses, orgLearners, orgCoaching, orgGradebook,
...
const learnerStatus = (s: string) =>
  s === 'completed' ? 'bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300'
    : s === 'at-risk' ? 'bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300'
 ...
 * a self-contained hub where every surface — people, courses, coaching, gradebook, funding,
 * documents and billing — is scoped to this one organisation. Nothing partner-wide is reachable
 * from here; the sidebar's "All organisations" is the only way back out. Seeded, functional.
 */
  ...
  return (
    <div className="space-y-6">
      ...
      {section === 'overview' && (
        <>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {[
              { label: 'People', href: `${base}/people`, icon: Users, sub: `${d.admins.length + d.coaches.length} staff · ${d.delegated.length} delegated` },
      ...
      {section === 'people' && (
        <div className="space-y-4">
          <Card className="p-5">
            <h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><Users className="h-4 w-4 text-primary" /> Staff</h3>
            <div className="space-y-2 text-sm">
              {[...d.admins, ...d.coaches].map((a) => (
                <div key={a.id} className="flex items-center justify-between gap-3 border-b border-border last:border-0 pb-2 last:pb-0">
                  ...
                  <Badge variant="secondary" className="capitalize text-[10px] shrink-0">{a.role.replace('_', ' ')}</Badge>
                </div>
              ))}
              {d.admins.length + d.coaches.length === 0 && <div className="text-muted-foreground">No staff accounts yet.</div>}
            </div>
          </Card>

          {d.delegated.length > 0 && (
            <Card className="p-5">
              ...
              <div className="space-y-2.5">
                {d.delegated.map((da) => (
                  <div key={da.id} className="rounded-lg border border-border p-3 text-sm">
                    <div className="flex items-center justify-between gap-2"><span className="font-medium">{da.name}</span><Badge variant="secondary" className="capitalize text-[10px]">{da.status}</Badge></div>
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {da.powers.map((p) => { const m = DELEGATABLE_POWERS.find((x) => x.key === p); return <span key={p} className="rounded bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground">{m?.label ?? p}</span>; })}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Card className="overflow-hidden">
            <div className="p-3 text-sm font-semibold flex items-center gap-2 border-b border-border"><GraduationCap className="h-4 w-4 text-primary" /> Learners <span className="text-muted-foreground font-normal">· {d.sub?.activeSeats ?? learners.length} active</span></div>
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
                <tr><th className="text-left p-3">Learner</th><th className="text-left p-3">Course</th><th className="text-left p-3">Progress</th><th className="text-left p-3">Status</th></tr>
              </thead>
              <tbody className="divide-y divide-border">
                {learners.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td className="p-3"><div className="flex items-center gap-2"><Progress value={l.progress} className="h-1.5 w-24" /><span className="text-xs tabular-nums text-muted-foreground">{l.progress}%</span></div></td>
                    <td className="p-3"><span className={cn('rounded px-2 py-0.5 text-[10px] font-medium capitalize', learnerStatus(l.status))}>{l.status.replace('-', ' ')}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
            <p className="p-3 text-xs text-muted-foreground">Showing a sample of the roster. Full learner management is delivered through the org's coaches.</p>
          </Card>
        </div>
      )}
      ...
      {section === 'courses' && (
        <Card className="overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
              <tr><th className="text-left p-3">Course</th><th className="text-left p-3">Modality</th><th className="text-right p-3">Enrolled</th><th className="text-left p-3">Avg progress</th><th className="text-left p-3">Status</th></tr>
            </thead>
            <tbody className="divide-y divide-border">
              {courses.map((c) => (
                <tr key={c.id}>
