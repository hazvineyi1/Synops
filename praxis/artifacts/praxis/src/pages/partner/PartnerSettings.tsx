...
/**
 * Partner Settings (learning-only): partner profile, branding entry point, notification preferences,
 * security posture and data export. No billing or funder configuration.
 */
export function PartnerSettings() {
  ...
  const [prefs, setPrefs] = useState({
    weeklyDigest: false, delegateActions: true, loginAlerts: true,
  });
  const toggle = (k: keyof typeof prefs) => setPrefs((p) => ({ ...p, [k]: !p[k] }));

  return (
    <div className="space-y-6">
      <PageHeader title="Settings" icon={Settings} subtitle="Partner profile, branding, notifications, security and data." />
        ...
        <TabsContent value="profile" className="mt-4 space-y-4">
          <Card className="p-5 space-y-4 max-w-2xl">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <label className="text-xs"><span className="mb-1 block font-medium text-muted-foreground">Contact email</span>
        ...
        <TabsContent value="notifications" className="mt-4">
          <Card className="p-5 max-w-2xl divide-y divide-border">
            {([
              ...
              ['delegateActions', 'Admin actions', 'Summarise actions taken by organisation admins.'],
              ['weeklyDigest', 'Weekly digest', 'A Monday summary of learning activity across all organisations.'],
