...
function clean(type: InteractionType, spec: ActivitySpec): ActivitySpec {
  ...
  if (type === "quiz") return { questions: (spec as QuizSpec).questions.filter((q) => ne(q.q)).map((q) => ({ q: q.q, img: (q as { img?: string }).img || undefined, options: q.options.filter((o) => ne(o.t)) })) } as ActivitySpec;
...
"function aximg(u){return u?'<img src=\"'+u+'\" style=\"max-width:100%;max-height:170px;border-radius:10px;margin:0 0 10px;display:block\"/>':'';}" +
"function render(){if(i>=qs.length){return done(Math.round(c/qs.length*100),{correct:c,total:qs.length});}var q=qs[i];ans=false;r.innerHTML=hd()+aximg(q.img)+'<div class=\"ax-q\">'+q.q+'</div><div id=o></div><div id=w></div><button class=ax-next id=n disabled>'+((i+1<qs.length)?'Next question':'See result')+'</button>';var o=document.getElementById('o');q.options.forEach(function(op,k){var b=document.createElement('button');b.className='ax-btn';b.innerHTML=op.t;b.onclick=function(){pick(k,b,q);};o.appendChild(b);});document.getElementById('n').onclick=function(){i++;render();};}" +
...
"function fimg(u){return u?'<img src=\"'+u+'\" style=\"max-width:100%;max-height:130px;border-radius:8px;margin-bottom:8px\"/>':'';}" +
"function render(){if(i>=cs.length){return done(Math.round(known/cs.length*100),{known:known,total:cs.length});}var card=cs[i];flipped=false;var p=Math.round(i/cs.length*100);r.innerHTML='<p class=\"ax-inst\">Read the prompt, tap the card to flip, then say if you knew it.</p><div class=\"ax-hd\"><span class=\"ax-chip\">Card '+(i+1)+' of '+cs.length+'</span><span class=\"ax-chip\">&#9989; '+known+' known</span></div><div class=\"ax-bar\"><i style=\"width:'+p+'%\"></i></div><div class=ax-card id=card>'+fimg(card.img)+card.front+'</div><div id=ctl></div>';document.getElementById('card').onclick=flip;ctl();}" +
