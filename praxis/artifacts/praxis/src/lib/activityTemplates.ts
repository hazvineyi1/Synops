...
"function render(){if(i>=qs.length){return done(Math.round(c/qs.length*100),{correct:c,total:qs.length});}var q=qs[i];ans=false;r.innerHTML=hd()+'<div class=\"ax-q\">'+q.q+'</div><div id=o></div><div id=w></div><button class=ax-next id=n disabled>'+((i+1<qs.length)?'Next question':'See result')+'</button>';var o=document.getElementById('o');q.options.forEach(function(op,k){var b=document.createElement('button');b.className='ax-btn';b.innerHTML=op.t;b.onclick=function(){pick(k,b,q);};o.appendChild(b);});document.getElementById('n').onclick=function(){i++;render();};}" +
