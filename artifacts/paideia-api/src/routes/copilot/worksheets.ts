...
import { z } from "zod";
import { db, worksheetsTable, classesTable, studentsTable, materialsTable } from "@workspace/paideia-db";
...
router.post("/", requireQuota, async (req, res) => {
  ...
  try {
    ...
    const prompt = worksheetPrompt({ ...parsed.data, classLearningProfile, sourceMaterial });
