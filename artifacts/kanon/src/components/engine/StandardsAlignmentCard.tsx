...
import {
  ...
  type Objective,
} from "@workspace/kanon-api-client";
...
export function StandardsAlignmentCard({
  ...
  const { data: competencies } = useListCompetencies(activeFrameworkId ?? 0, {
    query: { enabled: !!activeFrameworkId, queryKey: getListCompetenciesQueryKey(activeFrameworkId ?? 0) },
  });
  const { data: links } = useListCrosswalkLinks(projectId, {
    query: { enabled: !!projectId, queryKey: getListCrosswalkLinksQueryKey(projectId) },
  });
