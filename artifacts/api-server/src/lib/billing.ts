...
// What the subscription state means for access. A user is "pro" if they have an
// active/trialing paid subscription OR are within their free-trial window.
...
async function stripePost(path: string, params: Record<string, string | number | undefined>): Promise<any> {
...
export async function createCheckoutSession(opts: {
  ...
  // Reuse an existing customer when we have one, otherwise let Stripe make one
  // from the email (and we capture its id on the webhook).
